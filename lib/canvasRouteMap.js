  ...
  _createClass(Canvas, [{
    ...
    value: function animatePathWithImage(path) {
      ...
      function animate() {
        ...
        setTimeout(function () {
          timerId = requestAnimationFrame(animate.bind(_this));
          if (!path[i]) {
          ...
          if (i + step >= path.length) {
            ...
            _this.ctx.drawImage(_this.image, _this.canvasPosition.deltaX, _this.canvasPosition.deltaY, _this.initialImageWidth, _this.newImageHeight);
            _this.pathBuffer = [];
            i = 0;
            timerId = requestAnimationFrame(animate.bind(_this));
          } else {
